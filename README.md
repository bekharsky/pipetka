...
For a non-release build with the older GitHub toolchain, run the manually triggered `Legacy SDK Build` workflow. It uses the `macos-15` runner (Xcode 16.4 / macOS 15.5 SDK) and uploads an unsigned ZIP as an Actions artifact without creating a GitHub Release.
